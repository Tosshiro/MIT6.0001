...
def update_hand(hand, word):
    ...
    for i in word.lower():
        if hand_clone.get(i,0):
            hand_clone[i] -= 1

    #remove negative values in hand
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while running:
        ...
        #  End loop if run out of letters
...
def substitute_hand(hand, letter):
    ...
    sub_let = random.choice(
        [x for x in alphabet if x != letter and x not in hand]
    )

    for key in hand:
        if letter == key:
            if sub_let not in hand:
                #Substituting letter
                hand_clone[sub_let[0]] = hand[key]
